...
@section('form_action', route('sizes.update', $size->id))
        ...
        @method('PUT')
            ...
            <div class="card card-custom mb-2">
                <div class="card-header card-header-tabs-line">
                    @include('admin.components.breadcrumb', ['module' => 'sizes', 'action' => 'edit'])
                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="{{ $locale . '[title]' }}"
                                            ...
                                            value="{{ old($locale . '.title', $size->translate($locale)->title) }}">
                                    </div>
                                </div>
                                @error($locale . '.title')
            ...
            <div class="card card-custom">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select form-select-lg" name="store_id" id="store">
                                    ...
                                    @foreach ($stores as $store)
                                        <option value="{{ $store->id }}" {{ old('store_id',$size->store->id)==$store->id? 'selected' : '' }}>{{ $store->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('store_id')
                ...
                <div class="card-footer mb-5 mt-5">
                    ...
                    <a href="{{ route('sizes.index') }}" class="btn btn-outline-danger font-weight-bold">{{__('general.cancel')}}</a>
                </div>
            </div>
        </div>
    </div>
